...
- Atualiza mensagens de ajuda para usar `npx pm2`
...
| Arquivo | Alteração |
|---------|-----------|
| `start-pm2.bat` | PM2 local via `node_modules\.bin\pm2.cmd` |
| `scripts/install.bat` | PM2 local via `node_modules\.bin\pm2.cmd` |
...
| `npm run dev` (sem agente) | ✅ OK | API inicia sem erros |
...
| `start-pm2.bat` | ✅ OK | Usa PM2 local, não depende de PATH |
